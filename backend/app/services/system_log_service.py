...
class SystemLogService:
    ...
    async def create_system_log(self, log: SystemLogCreate, current_user: Optional[Users] = None) -> SystemLogOut:
        """
    ...
    async def get_system_log_by_id(self, log_id: int) -> Optional[SystemLogOut]:
        """
    ...
    async def get_system_logs(self, skip: int = 0, limit: int = settings.DEFAULT_PAGE_SIZE) -> List[SystemLogOut]:
        """
    ...
    async def get_system_logs_by_user(self, user_id: int, skip: int = 0, limit: int = settings.DEFAULT_PAGE_SIZE) -> List[SystemLogOut]:
        """
...
async def create_log(
    ...
    return await service.create_system_log(log, current_user)

@router.get("/{log_id}", response_model=SystemLogOut)
@require_permissions([Permission.VIEW_SYSTEM_LOG])
async def get_log(
    log_id: int,
    service: SystemLogService = Depends(get_system_log_service)
):
    """Retrieve a system log by ID. Requires VIEW_SYSTEM_LOG permission."""
    return await service.get_system_log_by_id(log_id)
...
async def list_logs(
    ...
    """List system logs with pagination. Requires VIEW_SYSTEM_LOG permission."""
    return await service.get_system_logs(skip, limit)
...
async def list_user_logs(
    ...
    """List system logs for a specific user. Requires VIEW_SYSTEM_LOG permission."""
    return await service.get_system_logs_by_user(user_id, skip, limit)