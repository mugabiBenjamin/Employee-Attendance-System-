...
async def login_for_access_token(
    ...
    try:
        ...
        # Create tokens with stricter expiry
        access_token = create_access_token({"sub": str(user.user_id)}, expires_delta=1800)  # 30 minutes
        refresh_token = create_refresh_token({"sub": str(user.user_id)}, expires_delta=604800)  # 7 days
...
async def refresh_access_token(
    ...
    try:
        ...
        # Create new tokens
        access_token = create_access_token({"sub": str(user.user_id)}, expires_delta=1800)  # 30 minutes
        new_refresh_token = create_refresh_token({"sub": str(user.user_id)}, expires_delta=604800)  # 7 days
